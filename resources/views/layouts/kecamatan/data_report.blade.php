<table class="table table-hover text-center" style="zoom: 0.75">
    <div class="card-header d-flex justify-content-end" style="zoom: 0.75">
        <a href="{{ route('report/kecamatan/excel/donwload') }}">
            <button id="exportBtn" class="btn btn-success">Export Kecamatan</button>
        </a>
    </div>
    <thead>
        <tr>
...
@foreach ($kecamatan as $data)
        ...
        @if ($current_dapil !== '')
            <tr style="background-color: #f5f5f5;">
                ...
                @foreach ($tim as $partai)
                    <td><strong>{{ number_format($dapil_party_totals[$partai->id] ?? 0) }}</strong></td>
                @endforeach
    ...
    <tr>
        ...
        @foreach ($tim as $partai)
            ...
            @endphp
            <td>{{ number_format($partai_count) }}</td>
        @endforeach
...
<tr style="background-color: #c6e0f5">
    ...
    @foreach ($tim as $partai)
        <td><strong>{{ number_format($grand_party_totals[$partai->id]) }}</strong></td>
    @endforeach
